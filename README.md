...
parc_model = parc.PARC(x_data=x_data, y_data_true=y_data_true) # instantiate the PARC class
# parc_model = parc.PARC(x_data=x_data) # when no 'true labels' are available
...
graph = parc_model.knn_graph_full()
...
y_data_true = [] # true labels
with open(file_path_y, "rt") as file:
    for line in file:
      y_data_true.append(line.strip().replace('\"', ''))   
...
parc_model.run_parc()
y_data_true = parc_model.y_data_true
```
...
y_data_true = list(pd.read_csv(file_path_y, header=None)[0]) # list of cell-type annotations

# Run PARC on 1.1M cells
# Note: jac_weighted_edges can be set to false which provides an unweighted graph to leiden and
# offers some speedup

parc_model = parc.PARC(x_data, y_data_true=y_data_true, jac_weighted_edges=False)
parc_model.run_parc() # run the clustering
y_data_pred = parc_model.y_data_pred

```

### Example 6: Large-scale (70K subset and 1.1M cells) Lung Cancer cells rare (multi-ATOM imaging cytometry based features)
...
| ---------- |----------|
| `y_data_pred` | (list) length n_samples of corresponding cluster labels |
